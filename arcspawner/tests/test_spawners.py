...
async def test_torque(db, event_loop):
    ...
    batch_script_re_list = [
        re.compile(
            r"^PROLOGUE.*^arcspawner-singleuser singleuser_command.*^EPILOGUE",
            re.S | re.M,
        ),
...
async def test_slurm(db, event_loop):
    ...
    batch_script_re_list = [
        re.compile(
            r"PROLOGUE.*srun arcspawner-singleuser singleuser_command.*EPILOGUE", re.S
        ),
...
async def test_condor(db, event_loop):
    ...
    batch_script_re_list = [
        re.compile(r"exec arcspawner-singleuser singleuser_command"),
